...
public class CommandLineJobRunner
{
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    OptionSpec<String> overrideOpt = parser.acceptsAll(
        asList("o", "override"),
    ...
    for (String override : options.valuesOf(overrideOpt)) {
      ...
      if (pieces.length != 2) {
        Utils.croak(
            "Invalid property override: '" + override
    ...
    FlowManager allFlows = new RefreshableFlowManager(
        jobManager,
              ...
              public void completed(Status status)
              {
                if (status == Status.FAILED) {
                  ...
                  if (exceptions == null || exceptions.isEmpty()) {
                    System.out
                        .println(
                            "flowToRun.getExceptions() was null/empty when it should not have been.  Please notify the Azkaban developers."
                        );
                  }
